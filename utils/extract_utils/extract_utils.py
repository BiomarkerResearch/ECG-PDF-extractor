...
def create_measurement_points(lead_list, number_of_points):
    ...
    # Pre-smooth Y values to remove PDF quantization noise before Akima interpolation.
    # Linear interpolation previously hid this noise by producing flat-line segments (holding constant
    # values between sparse points). Akima exposes it as baseline jitter. A Savitzky-Golay filter on
    # the sparse source points removes quantization steps while preserving QRS morphology.
    from scipy.signal import savgol_filter
    if len(y_sorted) >= 11:
        y_smooth = savgol_filter(y_sorted, window_length=11, polyorder=2)
    elif len(y_sorted) >= 5:
        y_smooth = savgol_filter(y_sorted, window_length=5, polyorder=2)
    else:
        y_smooth = y_sorted
    ...
    if len(x_sorted) >= 4:
        interp = Akima1DInterpolator(x_sorted, y_smooth)
        result = interp(target_x)
    else:
        # Fallback to linear interpolation for very short segments (< 4 points)
        from scipy.interpolate import interp1d
        interp_fn = interp1d(x_sorted, y_smooth, kind='linear', fill_value='extrapolate')
