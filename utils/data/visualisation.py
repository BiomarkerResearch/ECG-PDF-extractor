...
def visualiseMulti(ecgs, scaling=1):
    ...
    for ecgid in ecgs:
        ...
        for leadid in ecg['leads']:
            ...
            offset = offset + 2500
        if imx.show() is False:
            logger.warning('No image viewer found - overview PNG for "%s" not displayed.', ecgid)
    ...
    for leadname in ecg.columns:
        ...
        for i in range(len(singlelead) - 1):
            ...
            draw.line((i * scaling, (-singlelead[i] + 1250) * scaling, (i + 1) * scaling,
                      ...
                      , fill=0, width=2)
        if ims.show() is False:
            logger.warning('No image viewer found - PNG for lead "%s" not displayed.', leadname)
...
def visualiseIndividualinMPL(ecg):
    ...
    for leadname in ecg.columns:
        # iterating through all leads to generate a single image per ECG
        # Generating an image for every lead
        ax = ecg.plot(kind='line', y=[leadname])
        plt.show()
        # Close the figure after display to prevent unbounded figure/memory buildup
        # when debugging long batches.
        plt.close(ax.figure)
