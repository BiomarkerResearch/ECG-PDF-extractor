...
from utils.metadata.extract_metadata import detect_manufacturer, CSV_COLUMNS
...
class ExecutionRunner:
    def __init__(self, config_path=None, path_source=None, path_sink=None):
        ...
        self.initialize_logger(loglevel='INFO')
    ...
    def _run_extractor(self, manufacturer, path_source, path_sink, filenames=None):
        """Run the appropriate extractor for a group of PDFs. Returns the extractor instance."""
        params = {
        ...
        ext.extract()
        logging.info('%s PDF extraction successful (%d files)', manufacturer, len(filenames or []))
        return ext

    def _write_consolidated_records(self, records, path_sink):
        """Write one consolidated ECG_records.csv for all processed groups.

        Written even when empty so stale metadata from previous runs cannot survive.
        """
        df = pd.DataFrame(records, columns=CSV_COLUMNS)
        out_path = os.path.join(path_sink.rstrip('/'), 'ECG_records.csv')
        df.to_csv(out_path, index=False)
        logging.info('Wrote %d metadata records to %s', len(df), out_path)
    ...
    def pre_processing(self):
        ...
        if self.IS_PDf:
            path_source = (
                ...
                else os.path.join(base_dir, 'data', 'original_ecgs')
            )
            path_sink = (
                self.path_sink_override
                if self.path_sink_override
                else os.path.join(base_dir, 'data', 'extracted_ecgs')
            )

            checkpathsandmake(path_sink)
            checkpathsandmake(path_source)

            # Group all PDFs in the (brand-neutral) source folder by detected manufacturer.
            logging.info('Detecting manufacturers …')
            ...
            # Explicit manufacturer selection filters to one group; 'Auto' runs all detected groups.
            if self.manufacturer in ('Schiller', 'Cardiosoft'):
                selected = [self.manufacturer]
                for other in ('Schiller', 'Cardiosoft'):
                    if other != self.manufacturer and groups[other]:
                        logging.warning(
                            '%d %s PDF(s) skipped (manufacturer filter = %s)',
                            len(groups[other]), other, self.manufacturer,
                        )
            else:
                selected = ['Schiller', 'Cardiosoft']
            ...
            if self.new_extraction:
                for mfr in selected:
                    if groups[mfr]:
                        ext = self._run_extractor(mfr, path_source, path_sink, groups[mfr])
                        if ext is not None:
                            all_records.extend(ext.records)
                # Single consolidated write AFTER all groups (extractors only collect)
                self._write_consolidated_records(all_records, path_sink)
            else:
        ...
        if self.IS_PDf and path_sink:
            ...
            missing_meta = sorted(set(derived_ecgs.keys()) - set(combined_records.keys()))
            if missing_meta:
                logging.warning(
                    '%d record(s) have waveform data but no metadata row in ECG_records.csv: %s',
                    len(missing_meta), missing_meta,
                )
