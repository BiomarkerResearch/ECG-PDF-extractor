...
from utils.extract_utils.extract_utils import rotate_origin_only, scale_values_based_on_eich_peak, \
    create_measurement_points, adjust_leads_baseline, preprocess_page_content, extract_graphics_string, \
    extract_eichzacke_from_page_bytes, clip_preamble_points, common_time_window
...
class CardiosoftExtractor(AbstractExtractor):
    ...
    def __init__(self, params):
        ...
        # Metadata records collected during extract(); consolidated CSV is
        # written by the runner after all extractor groups finish.
        self.records = []
    ...
    def extract(self):
        ...
        for file_name in tqdm(all_files, disable=_FROZEN):
            ...
            try:
                ...
                if lead_list is not None:
                    ...
                    # Rotate + calibrate all leads first, then resample onto the
                    # COMMON time window so all leads stay temporally aligned.
                    rotated_scaled = []
                    for lead in lead_list:
                        tmp = [rotate_origin_only(float(t[0]), float(t[1]), rotation_angle)
                               for t in lead]
                        scaled = scale_values_based_on_eich_peak(tmp, gamma)
                        rotated_scaled.append(np.array(scaled))

                    t_lo, t_hi = common_time_window(rotated_scaled)
                    target_x = np.linspace(t_lo, t_hi, self.number_of_points)

                    new_lead_list = [
                        create_measurement_points([tuple(p) for p in a],
                                                  self.number_of_points,
                                                  target_x=target_x)
                        for a in rotated_scaled
                    ]
        ...
        # Collect records; the runner writes one consolidated ECG_records.csv
        # after all extractor groups finish (prevents overwrite between groups).
        self.records = records

    def _write_ecg_records(self, records):
        """Write consolidated ECG_records.csv (standalone/__main__ usage only)."""
            ...
            for p in range(len(reader.pages)):
                ...
                if not is_cover_page:
                    ...
                    if isinstance(contents, list):
                        if not contents or contents[0] is None:
...
if __name__ == '__main__':
    path_source = '../data/original_ecgs/'
    path_sink = '../data/extracted_ecgs/'
    ...
    tmp.extract()
    tmp._write_ecg_records(getattr(tmp, 'records', []))
