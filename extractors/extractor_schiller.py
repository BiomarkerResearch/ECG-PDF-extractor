...
from utils.extract_utils.extract_utils import rotate_origin_only, scale_values_based_on_eich_peak, \
    create_measurement_points, adjust_leads_baseline, extract_eichzacke_from_page_bytes, \
    parse_schiller_waveforms, common_time_window
        ...
        for file_name in tqdm(all_files, disable=_FROZEN):
            ...
            try:
                ...
                if lead_list is not None:
                    # Calibrate all leads first, then resample onto the COMMON time
                    # window so all leads stay temporally aligned.
                    scaled_leads = []
                    for lead in lead_list:
                        tmp_lead = [
                            rotate_origin_only(float(t[0]), float(t[1]), math.radians(0))
                            for t in lead
                        ]
                        scaled = scale_values_based_on_eich_peak(tmp_lead, gamma)
                        scaled_leads.append(np.array(scaled))

                    t_lo, t_hi = common_time_window(scaled_leads)
