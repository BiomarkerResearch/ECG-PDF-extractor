...
class MainWindow(QMainWindow):
    ...
    def _refresh_pdf_list(self):
        ...
        self._detect_manufacturers()

    def _detect_manufacturers(self):
        """Scan all PDFs in input folder and show detection summary."""
        if not self.input_folder:
            return
        ...
        pdfs = sorted(p for p in Path(self.input_folder).glob("*.pdf"))
        for pdf_path in pdfs:
            with open(pdf_path, 'rb') as f:
                mfr = detect_manufacturer(f.read())
            counts[mfr or 'Unknown'] += 1

        self.lbl_det_schiller.setText(f"Schiller: <b>{counts['Schiller']}</b>")
        self.lbl_det_cardio.setText(f"Cardiosoft: <b>{counts['Cardiosoft']}</b>")
        self.lbl_det_unknown.setText(f"Unknown: <b>{counts['Unknown']}</b>")
